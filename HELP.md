...
Полезные: `UPLOADS_DIR` — папка для загружаемых файлов (по умолчанию
`./uploads` в корне проекта; относительный путь всегда считается от корня,
а не от каталога запуска API).
...
- `attached_assets/` — четыре исходные спецификации проекта и временные новые вложения; периодически очищайте всё, кроме спецификаций
