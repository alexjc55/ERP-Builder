 ...
 *
 * This is purely cosmetic metadata — no record values are read, so there is
 * no RBAC leak: the rules only reveal the source field/status color config,
 * which any authenticated user can already see via the fields endpoints.
 */
