...
- **Dashboard pages** — a page can be a "dashboard" page type, mutually exclusive with entity-binding and mirror (enforced on every page-type-changing path against the effective final state). Renders admin-defined widgets; each widget is one or more metrics (count/sum over an entity, optionally restricted to statuses; sum over a numeric field) optionally combined via a client-side formula. Totals are admin-authoritative (real totals computed server-side, bypassing the viewer's row/field data perms) but only shipped to viewers authorized for the widget — page access + per-widget role visibility are checked before metrics are computed. Inline "Настроить" edit mode for admins, gated by the existing `pages` RBAC cap (no new cap). Memory: `dashboard-page-type.md`.
