...
- **Record Views (Views Engine)** — saved per-entity views (filters/sorts/search/visibleFields) backed by a POST query endpoint with type-aware filtering in Postgres. Views carry role-based visibility (`visibleRoleIds`) enforced server-side (role-invisible view → 404). Memory: `record-views-query.md`.
- **Pivot reports (Сводная таблица)** — a `pivot` view type for live interactive cross-tabs: rows + optional cols dimension (entity field, page-local field, or status; date fields bucket by year/quarter/month/day), count/sum measure, with row/col/grand totals. Opt-in per entity (`pivotEnabled`) + per field, all enforced server-side. Permission-scoped to the viewer (own-rows + hidden fields + hidden statuses), NOT admin-authoritative. Records page has a Таблица⇄Сводная toggle driven by the live filter bar. Also available as the entity's **default-view** pivot config (no named view required, with its own `visibleRoleIds` role-visibility gate enforced server-side; the request's `viewId` is a server-validated capability token, never a trusted branch switch) and as an **admin-authoritative dashboard widget** (`widgetType: "pivot"`, role-gated, sharing the same `computePivot` core; pivot opt-in re-checked at compute time), and as a dedicated **pivot PAGE type** (`isPivot` + `pivotEntityId` + `pivotConfigJson`, mutually exclusive with dashboard/mirror/bound-entity on create and update): also admin-authoritative (real totals over ALL the entity's records, the same for everyone with page access — the only boundary is page access, re-checked before compute), with three settings — filter/search/status quick-filters (NO sort: a pivot is aggregated so a record-level sort is a no-op), pivot config source (entity default / a pivot view / custom inline editor), and the existing role→page-access RBAC (no new permission). No export. Memory: `pivot-report-mode.md`.
...
- **Dashboard pages** — a page can be a "dashboard" page type, mutually exclusive with entity-binding and mirror (enforced on every page-type-changing path against the effective final state). Renders admin-defined widgets in a configurable cell grid (per-widget `gridW`/`gridH` spans). A widget is either a **metric** widget (one or more count/sum metrics over an entity, optionally restricted to statuses; sum over a numeric field; optionally combined via a client-side formula) or a **chart** widget (bar/line/area/pie/donut over grouped aggregation — count or sum, grouped by status or a field value, rendered with recharts). All widget values are admin-authoritative (real totals computed server-side, bypassing the viewer's row/field data perms) but only shipped to viewers authorized for the widget — page access + per-widget role visibility are checked before values are computed. Inline "Настроить" edit mode for admins, gated by the existing `pages` RBAC cap (no new cap). Memory: `dashboard-page-type.md`.
- **File Trash (корзина файлов)** — a recycle bin for LOCAL (object-storage) files removed from records. When a record is deleted or a file field is cleared/replaced, the local file is recorded in the `deleted_files` table (origin entity/field/record/who/when snapshot) and the physical object is kept until purged. Google Drive and pasted-link values are never tracked or deleted. The trash screen (`/admin/file-trash`) is superAdmin-only; "restore" = download only (no re-attach); permanent delete/purge drops the row only after the physical object is removed. Removing a file from a field now asks for confirmation. Trash recording is best-effort and never blocks the record op. Memory: `file-trash.md`.
- **Dependent (cascading) fields** — a `text` field can depend on another field of the same entity (`dependencyConfigJson.dependsOnFieldKey`). Its picker suggests the distinct existing values scoped to the row's parent chain, with create-time dedupe (exact=reuse, case-only collision=reject) and scoped rename/merge of values. Disabled until the parent is set; changing a parent clears its children at every edit surface. Two POST endpoints (`dependent-values`, `rename-value`) both re-apply the records read boundary. Memory: `dependent-fields.md`.
- **Cross-field validation (fill) rules** — a per-entity-field `validationRulesJson`: a value in THIS field (any non-empty, or only chosen `applyToValues` for select fields) is allowed only if another field satisfies an operator (empty/notEmpty/equals/notEquals/gt/lt/gte/lte/between). Otherwise the save is blocked (422) with an auto-generated Russian message. Distinct from cosmetic conditional formatting. Hard server boundary re-applied on EVERY record-write path (records create/update on final values, automations create/set_field, dependent rename-value). Phase 1 = block + message only; page-local fields out of scope. Memory: `validation-fill-rules.md`.
- **Mirror-page grouping** — a mirror page can group its records table by a source-entity field (`groupByFieldKey`, incl. single-link relations): collapsed group rows with server-computed count + sums (all `showColumnTotal` columns incl. page-local), accordion expand (one group at a time) into normal inline-editable rows; null/empty bucket last. Relation group labels re-apply the linked-entity field boundary (hidden → label withheld, opaque id key only). Memory: `mirror-page-grouping.md`.
- **Records column totals** — a numeric field can opt into `showColumnTotal`; `records/query` returns `numericTotals` summed server-side over the full filtered set (records are paginated) and the table renders a totals row above the header. Only summed for visible numeric fields, preserving the hidden-field boundary. Memory: `dashboard-page-type.md`.
