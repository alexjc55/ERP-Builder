...
for (const p of [...find("эпокол"), ...find("логист")]) {
  ...
  for (const f of pf.filter(f=>f.pageId===p.id)) console.log("  pf", f.id, f.fieldKey, f.fieldType, JSON.stringify(f.nameJson), "cfg:", JSON.stringify(f.configJson ?? null)?.slice(0,400));
}
