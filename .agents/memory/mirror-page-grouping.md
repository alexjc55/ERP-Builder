...
- **No STALE-GROUP flash when expanding/switching a group:** the single-accordion row set is server-narrowed to the expanded group, but `records` is manual state that keeps the PREVIOUS fetch's rows (the collapsed = FULL set, or the previously-open group) until the new fetch resolves. Rendering rows the moment a group is open therefore flashes the wrong rows under the new header. Rule: the expanded rows (and pagination footer) must only render once the LAST COMPLETED fetch was for the CURRENTLY-targeted group selection — stamp each successful fetch with a group signature and gate the row render on that signature matching the current one, showing a spinner in the gap. The signature must be computed identically at fetch-time and render-time and cover all three modes (none / single group / expand-all). **Why:** the `hasLoadedRecords` no-flicker rule intentionally keeps the old table mounted across refetches — good for inline edits (same group sig → no blink) but it's exactly what leaks the other-group rows on a group CHANGE, so the group-change case needs this signature gate ON TOP of it.
