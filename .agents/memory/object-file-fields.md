...
- **The uploads root is workspace-anchored, never cwd-anchored.** Find the monorepo root via `pnpm-workspace.yaml`; resolve the default `uploads` and relative `UPLOADS_DIR` from there. pnpm filtered scripts run with the package as cwd, so using `process.cwd()` directly creates `artifacts/api-server/uploads`. Startup safely merges that legacy location into the canonical root without overwriting files.
