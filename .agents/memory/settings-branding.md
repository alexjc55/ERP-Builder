...
- **Why this is safe:** the object path comes from the DB (admin-set), never from request input, so a caller cannot point it at arbitrary files. The route branches to guarded local-path resolution or legacy Object Storage based on the stored scheme.
...
- **Totals strip (numericTotals row above the header):** the strip is part of the table GRID, not a separate box. Each totals `<th>` KEEPS the vertical column separator (the global index.css `border-right` rule — so do NOT set `border:"none"`, which kills it) and ONLY removes horizontal lines via `borderTop:"none"` + `borderBottom:"none"` (the table has no horizontal row lines). The aggregate fill spans the WHOLE cell — `backgroundColor` on the `<th>` (fill = `totalFillColor` or `#d1fae5` emerald-100; text = `totalTextColor` or `#047857` emerald-700) — NOT a chip span. Empty cells are `backgroundColor:"transparent"` but still show their vertical separator. Pass the resolved fill color into `pinStyle` for sticky cells. **Why:** the user explicitly wants the totals to read as the top rows of the same grid (vertical lines kept, full-cell fill), with only horizontal lines gone; a chip-in-empty-box and a full 4-side box border were both rejected. **Open caveat:** the strip is still the first row inside the bordered `<Card>`, so the card's outer outline wraps it; floating it fully above the card outline needs a separate width-synced layer (browser table auto-sizing means a detached totals table can't reliably align with auto-width columns) — not done.
