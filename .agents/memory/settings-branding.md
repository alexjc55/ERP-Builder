...
- **Totals strip (numericTotals row above the header):** the whole strip is chrome-free — EVERY totals `<th>` is `backgroundColor:"transparent"` + `border:"none"` (the inline `border:none` is what defeats the global per-cell `border-right` separator locally) and passes `"transparent"` to `pinStyle`. The aggregate value is rendered as a SELF-CONTAINED rounded chip `<span>` (fill = `totalFillColor` or `bg-emerald-100`, text = `totalTextColor` or `text-emerald-700`), NOT a bordered/filled table cell. **Why:** the records table has NO horizontal lines anywhere; a bordered/box totals cell (the old approach) looked like a "frame above the header" and was rejected by the user. Keep totals lines-free. **Open caveat:** the strip is still the first row inside the bordered `<Card>`, so the card's outer outline technically wraps the (now transparent) strip; fully floating the totals above the card outline needs a separate width-synced layer (not yet done).
