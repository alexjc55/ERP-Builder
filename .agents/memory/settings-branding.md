...
- `app_settings.tableStyle` (plain/striped/striped_bold) plus two optional hex colors `tableStripeColor` + `tableHeaderColor` drive how EVERY records table renders platform-wide. Admin-edited on `/settings` (same `settings` cap).
