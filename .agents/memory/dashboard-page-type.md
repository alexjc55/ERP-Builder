...
- `widgetType: "notes"` is a fourth widget shape. Like all widgets it lives on ANY page and rides the embedded `DashboardView` automatically (so it appears atop normal/mirror records pages too — no separate plumbing). `config.notes` is JSONB (no new DB columns), added to OpenAPI `WidgetConfig` + `DashboardWidgetData`.
- Two modes (`config.notes.kind`): `richtext` (a single sanitized HTML blob) and `table` (`cols` + row-major `cells: NoteCell[][]`).
- **Rich-text is double-sanitized:** server sanitizes on EVERY create/update (`sanitizeNotesHtml`, explicit tag/attr/style allowlist + safe link schemes) AND the client re-sanitizes with DOMPurify before `dangerouslySetInnerHTML`. Never trust stored HTML on read — a direct-DB edit could inject. Editor is Tiptap v3; StarterKit v3 already bundles underline/link/lists, so only add TextStyle+Color+TextAlign separately (adding Underline/Link again throws duplicate-extension). Use `immediatelyRender:false`.
- **Table cell = static text OR dynamic.** A dynamic cell has 1+ `sources` (each keyed for formula refs) + optional per-cell `formula` + `format`. A source is either `sourceKind:"metric"` (count/sum + statusIds + relationId — identical shape to a metric widget, reuse `validateMetricLike`/`computeMetric`) or `sourceKind:"record"` (entityId+recordId+fieldKey → that one record's stored value). Server computes all source values **admin-authoritative** (same as every widget total — gated only by page-access + per-widget role visibility, NOT the viewer's row/field RBAC), ships `{key→value}` per cell; the client evaluates the formula (or shows the first value) exactly like metric widgets via `evaluateFormula`.
- **Record-source caveat (intentional, not a bug):** a record-source cell can surface a concrete field value of an entity/field the viewer normally can't see — admin-authoritative by design, consistent with metric/chart/status-column widgets. The boundary is widget visibility, not field RBAC.
