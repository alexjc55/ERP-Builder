...
- [Multiple roles per user](multi-role-users.md) — roles are additive (most-permissive union); every single-role check (field access, transitions, impersonation) must use the full roleIds set on BOTH server and client; write contract = non-empty roleIds, roleIds[0]=primary.
