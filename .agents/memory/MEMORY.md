...
- [Pivot report mode](pivot-report-mode.md) — pivot as a viewType (perm-scoped) AND a dashboard widget (admin-authoritative twin) sharing one computePivot core; 3 server opt-in gates incl. compute-time pivotEnabled recheck; dashboard pivots reject page-local sources; PIVOT_DIM_TYPES stays in sync client↔server.
