...
External DOCX package relationships remain fail-closed except for standard Transitional/Strict OOXML hyperlink relationships targeting valid `http`, `https`, `mailto`, `tel`, or `whatsapp` URIs.

**Why:** Business templates commonly contain clickable website, email, phone, and WhatsApp links; hyperlink relationships are not fetched during sandboxed rendering. Broad relationship-type exceptions would reopen SSRF/local-file risks.
