...
## User-field formatting must compare the RAW stored id, not the formula-scope name
**Rule:** the `getValue` feeding `computeRowFormatting` must NOT read `user` fields from the formula-resolved values map. `resolveFormulaValues` substitutes user id → display name (so formulas show names), but format rules store `String(id)` — matching against the name silently never fires. Branch on `fieldType === "user"` and return the raw `allValues[key]`.
**Why:** the formula name-substitution feature regressed user-field formatting exactly this way — the June id-comparison fix was correct, but the later shared `formulaValues` source reintroduced the id-vs-name mismatch upstream.
**How to apply:** any new per-row computation (formatting, filters, totals) must decide per field type whether it needs the raw stored value or the display-resolved one; never assume one shared "values" map serves both.
