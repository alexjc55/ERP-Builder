...
Link changes must emit `record.updated` (post-commit, both ends incl. previously-linked on replace) on EVERY link-write surface. This is now enforced structurally: all link-write routes go through the shared `lib/record-links.ts` (`replaceSingleRelationLink` for single-link replace + `emitLinkChangedEvents` for notification). Any NEW link-write surface must use these helpers — inlining the transaction/emission is how automations silently stopped firing from the record form once already.
