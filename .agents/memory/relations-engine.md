...
Link changes must emit `record.updated` (post-commit, both ends incl. previously-linked on replace) on EVERY link-write surface — there are THREE: `POST/DELETE /records/:id/links` (relations.ts), the page-level `PUT /pages/:id/related-link`, and the entity-level `PUT /entities/:id/related-link` (the one the record create/edit form uses). Missing any one makes automations silently not fire from that UI path.
