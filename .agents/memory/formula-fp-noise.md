...
- `+` semantics: an empty operand (null/"") next to a numeric one counts as 0 (result stays a NUMBER so decimals apply); string concat only when a side is genuine text. **Why:** `{price}+{empty_field}` silently became a STRING, bypassing decimals rounding AND FP-noise cleanup.
